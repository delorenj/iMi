...
mod path_construction_validation {
    ...
    async fn test_config_path_construction_no_doubling() -> Result<()> {
        ...
        assert_eq!(repo_path, expected_repo, 
        ...
        assert!(!repo_path_str.contains("//"), 
            "Path should not contain double separators: {}", repo_path_str);
    ...
    async fn test_trunk_path_construction_no_doubling() -> Result<()> {
        ...
        let expected_trunk = config.root_path
            .join(repo_name)
            .join(format!("trunk-{}", config.git_settings.default_branch));
        
        assert_eq!(trunk_path, expected_trunk,
        ...
        assert!(!trunk_str.contains("//"), 
            "Trunk path should not contain double separators: {}", trunk_str);
        assert!(!trunk_str.contains(&format!("trunk-{}/trunk-{}", 
            config.git_settings.default_branch, config.git_settings.default_branch)), 
            "Trunk path should not contain repeated trunk segment: {}", trunk_str);
    ...
    async fn test_worktree_path_construction_no_doubling() -> Result<()> {
        ...
        assert_eq!(worktree_path, expected_worktree,
            "get_worktree_path should construct path without doubling: expected {:?}, got {:?}",
            expected_worktree, worktree_path);
        
        // Ensure no double separators or repeated segments  
        let worktree_str = worktree_path.to_string_lossy();
        assert!(!worktree_str.contains("//"), 
            "Worktree path should not contain double separators: {}", worktree_str);
        assert!(!worktree_str.contains(&format!("{}/{}", worktree_name, worktree_name)), 
            "Worktree path should not contain repeated worktree name: {}", worktree_str);
    ...
    async fn test_sync_path_construction_no_doubling() -> Result<()> {
        ...
        let expected_global = config.root_path
            .join(repo_name)
            .join(&config.sync_settings.global_sync_path);
        
        assert_eq!(global_sync, expected_global,
        ...
        let expected_repo_sync = config.root_path
            .join(repo_name)
            .join(&config.sync_settings.repo_sync_path);
            
        assert_eq!(repo_sync, expected_repo_sync,
        ...
        for (path, name) in [(global_sync, "global"), (repo_sync, "repo")] {
            ...
            assert!(!path_str.contains("//"), 
                "{} sync path should not contain double separators: {}", name, path_str);
        }
    ...
    async fn test_specific_path_doubling_bug_scenarios() -> Result<()> {
        ...
        for (path, desc) in [
            ...
            let path_str = path.to_string_lossy();
            assert!(!path_str.contains(".imi/.imi"), 
                "{} should not contain doubled .imi: {}", desc, path_str);
            assert!(!path_str.contains(".imi.imi"), 
                "{} should not contain .imi.imi: {}", desc, path_str);
        }
    ...
    async fn test_database_path_construction_validation() -> Result<()> {
        ...
        assert!(!db_str.contains("//"),
            "Database path should not contain double separators: {}", db_str);
        
        // Should not have repeated .db extensions
        assert!(!db_str.contains(".db.db"),
            "Database path should not contain doubled extension: {}", db_str);
    ...
    async fn test_config_path_validation() -> Result<()> {
        ...
        assert!(!config_str.contains("//"),
            "Config path should not contain double separators: {}", config_str);
        
        // Should not have doubled segments
        assert!(!config_str.contains("iMi/iMi"),
            "Config path should not contain doubled iMi segment: {}", config_str);
        
        // Should end with config.toml, not doubled
        assert!(!config_str.contains("config.toml.toml"),
            "Config path should not have doubled extension: {}", config_str);
        
        assert!(config_str.ends_with("config.toml"),
            "Config path should end with config.toml: {}", config_str);
    ...
    async fn test_path_canonicalization_no_doubling() -> Result<()> {
        ...
        for (repo_name, description) in test_cases {
            ...
            for (path, path_name) in [(&repo_path, "repo"), (&trunk_path, "trunk")] {
                ...
                assert!(!path_str.contains("//"), 
                    "{} path should not contain double separators: {}", path_name, path_str);
    ...
    async fn test_path_construction_regression_prevention() -> Result<()> {
        ...
        for (repo_name, worktree_name, description) in test_scenarios {
            println!("Testing scenario: {} (repo: '{}', worktree: '{}')", 
    ...
    async fn test_full_init_workflow_path_validation() -> Result<()> {
        ...
        let init_cmd = InitCommand::new(true); // Force to avoid existing config issues
        ...
        match init_result {
            Ok(result) => {
                ...
                assert!(!db_path_str.contains("//"), 
                    "Final database path should not have double separators: {}", db_path_str);
                assert!(!root_path_str.contains("//"), 
                    "Final root path should not have double separators: {}", root_path_str);
