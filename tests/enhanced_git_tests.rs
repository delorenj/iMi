...
use imi::git::{GitManager, GitCredentials};
...
impl GitTestUtils {
    ...
    pub async fn create_worktree_structure(&self, worktree_name: &str) -> Result<PathBuf> {
        ...
        fs::write(git_file, format!("gitdir: {}/.git/worktrees/{}", 
                  self.repo_path.display(), worktree_name)).await?;
...
async fn test_git_credentials_creation() -> Result<()> {
    ...
    // Test with SSH key
    let creds_ssh = GitCredentials::new(
        None,
        None,
        Some("/path/to/ssh/key".to_string()),
    );
...
async fn test_branch_management() -> Result<()> {
    ...
    for branch_name in branch_names {
        ...
        assert!(is_valid_branch, "Branch name should be valid: {}", branch_name);
    }
...
async fn test_authentication_scenarios() -> Result<()> {
    ...
    let auth_scenarios = vec![
        ("https", GitCredentials::new(
            Some("user".to_string()),
            Some("token".to_string()),
            None,
        )),
        ("ssh", GitCredentials::new(
            None,
            None,
    ...
    for (path, should_be_valid) in path_scenarios {
        ...
        if should_be_valid {
            ...
            assert!(path.is_empty() || path.contains(' ') || path.starts_with("../"), 
                   "Path should have invalid characteristics: {}", path);
        }
    }
...
async fn test_git_cleanup() -> Result<()> {
    ...
    for i in 0..3 {
        let _worktree_path = utils.create_worktree_structure(&format!("cleanup-test-{}", i)).await?;
    }
