...
impl TestEnvironment {
    ...
    pub async fn create_test_worktree(&self, repo_name: &str, worktree_name: &str) -> Result<PathBuf> {
        ...
        fs::write(git_file, format!("gitdir: {}/.git/worktrees/{}", 
                  self.root_path().join(repo_name).display(), worktree_name)).await?;
...
impl<'a> RepositoryBuilder<'a> {
    ...
    pub fn build(self) -> Repository {
        ...
        let repository = Repository {
            ...
            remote_url: self.remote_url.unwrap_or_else(|| format!("https://github.com/test/{}.git", self.name)),
        ...
        self.builder.repositories.insert(self.name, repository.clone());
        repository
    }
}
...
impl<'a> WorktreeBuilder<'a> {
    ...
    pub fn build(self) -> Worktree {
        ...
        let worktree = Worktree {
            ...
            branch_name: self.branch_name.unwrap_or_else(|| format!("feature/{}", self.worktree_name)),
            worktree_type: self.worktree_type,
            path: self.path.unwrap_or_else(|| format!("/tmp/{}/{}", self.repo_name, self.worktree_name)),
            created_at: now,
            updated_at: now,
            active: self.active,
...
impl<'a> ActivityBuilder<'a> {
    ...
    pub fn build(self) -> AgentActivity {
        ...
        self.builder.activities.insert(self.worktree_id, activity.clone());
        activity
    }
}
...
impl AssertionUtils {
    ...
    pub fn assert_recent_timestamp(timestamp: &DateTime<Utc>, context: &str) {
        ...
        assert!(
            ...
            context, timestamp, now, diff.num_seconds()
        );
    }
    ...
    pub fn assert_valid_uuid(uuid_str: &str, context: &str) {
        ...
        assert!(parsed.is_ok(), "{}: should be a valid UUID: {}", context, uuid_str);
    }
    
    /// Assert that a path exists and is accessible
    pub fn assert_path_accessible<P: AsRef<Path>>(path: P, context: &str) {
        let path = path.as_ref();
        assert!(path.exists(), "{}: path should exist: {}", context, path.display());
        
        // Check if readable
        let metadata = std::fs::metadata(path);
        assert!(metadata.is_ok(), "{}: path should be readable: {}", context, path.display());
    }
    ...
    pub fn assert_same_length<T, U>(collection1: &[T], collection2: &[U], context: &str) {
        assert_eq!(
            ...
            context, collection1.len(), collection2.len()
        );
    }
    
    /// Assert that a collection is not empty
    pub fn assert_not_empty<T>(collection: &[T], context: &str) {
        assert!(!collection.is_empty(), "{}: collection should not be empty", context);
    }
    ...
    pub fn assert_performance(duration_ms: u128, max_expected_ms: u128, context: &str) {
        assert!(
            ...
            context, max_expected_ms, duration_ms
        );
    }
}
...
impl MockDataGenerator {
    ...
    pub fn repository_names(count: usize) -> Vec<String> {
        let prefixes = vec!["web", "api", "core", "ui", "data", "auth", "admin", "mobile"];
        let suffixes = vec!["app", "service", "lib", "tool", "engine", "client", "server", "worker"];
        
        (0..count).map(|i| {
            if i < prefixes.len() * suffixes.len() {
                format!("{}-{}", prefixes[i % prefixes.len()], suffixes[i / prefixes.len()])
            } else {
                format!("repo-{}", i)
            }
        }).collect()
    ...
    pub fn worktree_names(count: usize) -> Vec<String> {
        ...
        let features = vec![
            "user-auth", "api-endpoints", "data-validation", "error-handling",
            "performance", "ui-improvements", "integration", "monitoring"
        ];
    ...
    pub fn remote_urls(repo_names: &[String]) -> Vec<String> {
        ...
        repo_names.iter().enumerate().map(|(i, name)| {
...
impl BenchmarkResult {
    ...
    pub fn print_summary(&self) {
        ...
        println!("  Duration - Min: {}ms, Max: {}ms, Avg: {}ms, Median: {}ms",
