...
use imi::database::{Database, Worktree, Repository, AgentActivity};
...
impl DatabaseTestUtils {
    ...
    pub async fn create_test_repository(&self, name: &str) -> Result<Repository> {
        self.database.create_repository(
            name,
            &format!("/tmp/{}", name),
    ...
    pub async fn create_test_activity(
        ...
        self.database.log_agent_activity(
            "test-agent",
            worktree_id,
            activity_type,
            Some("/tmp/test.rs"),
...
async fn test_repository_duplicate_names() -> Result<()> {
    ...
    let repo2 = utils.database.create_repository(
        "duplicate-repo",
        "/different/path",
...
async fn test_worktree_crud_operations() -> Result<()> {
    ...
    let worktree = utils.create_test_worktree("test-repo", "feature-branch").await?;
    ...
    // Test Read worktree
    let retrieved = utils.database.get_worktree("test-repo", "feature-branch").await?;
    assert!(retrieved.is_some());
    let retrieved = retrieved.unwrap();
    ...
    utils.database.deactivate_worktree("test-repo", "feature-branch").await?;
    let deactivated = utils.database.get_worktree("test-repo", "feature-branch").await?;
    assert!(deactivated.is_none(), "Deactivated worktree should not be retrieved");
...
async fn test_worktree_with_agent() -> Result<()> {
    ...
    let worktree = utils.database.create_worktree(
        "test-repo",
        "agent-worktree",
    ...
    let retrieved = utils.database.get_worktree("test-repo", "agent-worktree").await?;
    assert!(retrieved.is_some());
...
async fn test_worktree_invalid_repository() -> Result<()> {
    ...
    let result = utils.create_test_worktree("non-existent-repo", "feature").await;
...
async fn test_agent_activity_logging() -> Result<()> {
    ...
    let activities = utils.database.get_recent_activities(Some(&worktree.id), 10).await?;
...
async fn test_multiple_agent_activities() -> Result<()> {
    ...
    let _activity3 = utils.create_test_activity(&worktree.id, "committed").await?;
    
    // Test retrieval with limit
    let activities = utils.database.get_recent_activities(Some(&worktree.id), 2).await?;
...
async fn test_concurrent_database_operations() -> Result<()> {
    ...
    let worktrees = utils.database.list_worktrees(Some("concurrent-repo")).await?;
...
async fn test_database_error_scenarios() -> Result<()> {
    ...
    let result = utils.database.get_recent_activities(Some("non-existent-id"), 10).await?;
    assert_eq!(result.len(), 0, "Should return empty list for non-existent worktree");
    
    // Test deactivating non-existent worktree
    let result = utils.database.deactivate_worktree("non-repo", "non-worktree").await;
...
async fn test_worktree_unique_constraint() -> Result<()> {
    ...
    let worktree2 = utils.database.create_worktree(
        "test-repo",
        "duplicate",
        "different-branch",
        "fix",
    ...
    let retrieved = utils.database.get_worktree("test-repo", "duplicate").await?;
    assert!(retrieved.is_some());
    let retrieved = retrieved.unwrap();
...
async fn test_data_integrity() -> Result<()> {
    ...
    assert!(time_diff.num_seconds() < 60, "Created timestamp should be recent");
    assert!(time_diff.num_seconds() >= 0, "Created timestamp should not be in future");
...
async fn test_database_properties() -> Result<()> {
    ...
    let worktree = utils.create_test_worktree("deactivation-test", "test-branch").await?;
    
    // Verify it exists
    let before = utils.database.get_worktree("deactivation-test", "test-branch").await?;
    assert!(before.is_some());
    
    // Deactivate
    utils.database.deactivate_worktree("deactivation-test", "test-branch").await?;
    
    // Verify it's gone
    let after = utils.database.get_worktree("deactivation-test", "test-branch").await?;
