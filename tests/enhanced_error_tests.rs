...
async fn test_validation_error_scenarios() -> Result<()> {
    ...
    for invalid_name in invalid_repo_names {
        ...
        assert!(has_invalid_chars, "Name should be invalid: {}", invalid_name);
    }
...
async fn test_error_recovery_mechanisms() -> Result<()> {
    ...
    for (attempt, expected_delay) in backoff_delays.iter().enumerate() {
        ...
        assert_eq!(calculated_delay, *expected_delay, "Backoff delay mismatch at attempt {}", attempt);
    }
...
async fn test_error_handling_performance() -> Result<()> {
    ...
    assert!(error_creation_time.as_millis() < 10, 
    ...
    assert!(propagation_time.as_millis() < 100, 
           "Error propagation took too long: {}ms", propagation_time.as_millis());
