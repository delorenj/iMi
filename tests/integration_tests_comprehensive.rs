...
impl TestEnvironment {
    pub async fn new() -> Result<Self> {
        ...
        let worktree_manager = WorktreeManager::new(git_manager.clone(), database.clone(), config.clone());
    ...
    pub async fn create_mock_repo_structure(&self, repo_name: &str, trunk_branch: &str) -> Result<(PathBuf, PathBuf)> {
        ...
        fs::write(trunk_dir.join("Cargo.toml"), r#"
[package]
name = "test-repo"
version = "0.1.0"
edition = "2021"
...
impl EndToEndTests {
        ...
        match init_result {
            Ok(result_obj) => {
                if !result_obj.success {
                    result.failures.push(format!("Init failed: {}", result_obj.message));
                    result.failed += 1;
                    return Ok(());
                }
                ...
                if !config_exists || !repo_in_db {
                    result.failures.push("Not all components were properly initialized".to_string());
                    result.failed += 1;
                    return Ok(());
                }
            ...
            Err(e) => {
                result.failures.push(format!("Init execution failed: {}", e));
                result.failed += 1;
                return Ok(());
            }
        }
    ...
    async fn test_complete_init_from_root(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        match init_result {
            Ok(result_obj) => {
                if !result_obj.success {
                    result.failures.push(format!("Root init failed: {}", result_obj.message));
                    result.failed += 1;
                    return Ok(());
                }
                result.passed += 1;
            ...
            Err(e) => {
                result.failures.push(format!("Root init execution failed: {}", e));
                result.failed += 1;
                return Ok(());
            }
        }
    ...
    async fn test_force_reinitialize(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        let _first_result = init_cmd1.execute().await?;
        
        // Then do a force reinit
        let init_cmd2 = InitCommand::new(true);
        let second_result = init_cmd2.execute().await;
        ...
        match second_result {
            Ok(result_obj) => {
                if !result_obj.success {
                    result.failures.push(format!("Force reinit failed: {}", result_obj.message));
                    result.failed += 1;
                    return Ok(());
                }
                result.passed += 1;
            ...
            Err(e) => {
                result.failures.push(format!("Force reinit execution failed: {}", e));
                result.failed += 1;
                return Ok(());
            }
        }
    ...
    async fn test_init_with_existing_config(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        match init_result {
            Ok(result_obj) => {
                if !result_obj.success {
                    result.failures.push(format!("Init with existing config failed: {}", result_obj.message));
                    result.failed += 1;
                    return Ok(());
                }
                result.passed += 1;
            ...
            Err(e) => {
                result.failures.push(format!("Init with existing config execution failed: {}", e));
                result.failed += 1;
                return Ok(());
            }
        }
    ...
    async fn test_init_complex_structure(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        match init_result {
            Ok(result_obj) => {
                if !result_obj.success {
                    result.failures.push(format!("Complex structure init failed: {}", result_obj.message));
                    result.failed += 1;
                    return Ok(());
                }
                result.passed += 1;
            ...
            Err(e) => {
                result.failures.push(format!("Complex structure init execution failed: {}", e));
                result.failed += 1;
                return Ok(());
            }
        }
    ...
    async fn test_init_multiple_repositories(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        if !result1.success || !result2.success || !repo1_exists || !repo2_exists {
            result.failures.push("Multiple repository init failed".to_string());
            result.failed += 1;
            return Ok(());
        }
...
impl ComponentInteractionTests {
    ...
    pub async fn run(&self) -> Result<TestCategoryResult> {
        ...
        self.test_cross_component_error_handling(&mut result).await?;
    ...
    async fn test_config_database_interaction(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        if !custom_db_path.exists() {
            result.failures.push("Database not created at config-specified location".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_database_git_interaction(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        env.database.create_repository(
            "git-test-repo",
            "/path/to/repo",
        ...
        if repo_from_db.is_none() {
            result.failures.push("Repository not found in database after creation".to_string());
            result.failed += 1;
            return Ok(());
        }
        ...
        let git_operations_possible = self.test_git_operations_with_repo_data(&env.git_manager, &repo_data);
        
        if !git_operations_possible {
            result.failures.push("Git manager cannot work with database repository data".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_init_worktree_interaction(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        if !init_result.success {
            result.failures.push("Init failed, cannot test worktree interaction".to_string());
            result.failed += 1;
            return Ok(());
        }
        
        // Test worktree manager can find and work with initialized repository
        let can_manage_worktrees = self.test_worktree_management_after_init(&env.worktree_manager, "worktree-repo").await;
        
        if !can_manage_worktrees {
            result.failures.push("Worktree manager cannot work with initialized repository".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_cross_component_error_handling(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        match database_result {
            ...
            Ok(_) => {
                result.failures.push("Database creation succeeded with invalid path".to_string());
                result.failed += 1;
                return Ok(());
            }
        }
    ...
    async fn test_component_state_sync(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        env.database.create_repository("sync-repo", "/path/to/sync", "", "main").await?;
        
        // Modify state through different components
        let repo_from_db = env.database.get_repository("sync-repo").await?;
        
        if repo_from_db.is_none() {
            result.failures.push("Repository state not synchronized".to_string());
            result.failed += 1;
            return Ok(());
        }
        ...
        if !state_consistent {
            result.failures.push("Component states are not synchronized".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    // Helper methods
    fn test_git_operations_with_repo_data(&self, _git_manager: &GitManager, _repo_data: &Repository) -> bool {
        // Placeholder implementation
        true
    }

    async fn test_worktree_management_after_init(&self, _worktree_manager: &WorktreeManager, _repo_name: &str) -> bool {
        // Placeholder implementation
        true
    }
...
impl WorkflowTests {
    ...
    async fn test_developer_first_time_setup(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        if !init_result.success {
            result.failures.push(format!("First-time setup failed: {}", init_result.message));
            result.failed += 1;
            return Ok(());
        }
        ...
        if !setup_complete {
            result.failures.push("First-time setup incomplete".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_existing_project_integration(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        if !init_result.success || !files_preserved {
            result.failures.push("Existing project integration failed".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_multi_environment_setup(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        for env_name in &environments {
            ...
            if !init_result.success {
                result.failures.push(format!("Multi-environment setup failed for {}", env_name));
                result.failed += 1;
                return Ok(());
            }
        }
        ...
        let all_envs_registered = self.verify_all_environments_registered(&env, &environments).await;
        
        if !all_envs_registered {
            result.failures.push("Not all environments were registered".to_string());
            result.failed += 1;
            return Ok(());
        }
...
"#).await?;
        ...
        let init_cmd = InitCommand::new(true); // Force to simulate migration
        let migration_result = init_cmd.execute().await?;
        
        env::set_current_dir(original_dir)?;
        
        if !migration_result.success {
            result.failures.push("Migration workflow failed".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_recovery_workflow(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        // Initial setup
        let init_cmd1 = InitCommand::new(false);
        let _initial_result = init_cmd1.execute().await?;
        ...
        let init_cmd2 = InitCommand::new(true);
        let recovery_result = init_cmd2.execute().await?;
    ...
    async fn verify_complete_setup(&self, env: &TestEnvironment, repo_name: &str) -> bool {
        ...
        let repo_exists = env.database.get_repository(repo_name).await.unwrap_or(None).is_some();
        
        config_valid && repo_exists
    }

    async fn verify_all_environments_registered(&self, env: &TestEnvironment, environments: &[&str]) -> bool {
        for env_name in environments {
            let repo_name = format!("project-{}", env_name);
            if env.database.get_repository(&repo_name).await.unwrap_or(None).is_none() {
                return false;
            }
        }
        true
...
impl StateManagementTests {
    ...
    async fn test_state_persistence(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        env.database.create_repository("persist-repo", "/path", "", "main").await?;
        ...
        if repo.is_none() {
            result.failures.push("State not persisted across database reopening".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_state_recovery(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        env.database.create_repository("recovery-repo", "/path", "", "main").await?;
        
        // Test recovery mechanisms
        let recovery_successful = self.test_state_recovery_mechanisms(&env).await;
        
        if !recovery_successful {
            result.failures.push("State recovery mechanisms failed".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_concurrent_access(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        let concurrent_operations = vec![
            env.database.create_repository("concurrent-1", "/path1", "", "main"),
            env.database.create_repository("concurrent-2", "/path2", "", "develop"),
            env.database.create_repository("concurrent-3", "/path3", "", "feature"),
        ];
        ...
        if !all_succeeded {
            result.failures.push("Concurrent access operations failed".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_state_migration(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        // Create initial state
        env.database.create_repository("migration-repo", "/path", "", "main").await?;
...
impl CrossSystemTests {
    ...
    async fn test_filesystem_integration(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        if !fs_operations_successful {
            result.failures.push("Filesystem integration failed".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_environment_variable_handling(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        if !env_handling_successful {
            result.failures.push("Environment variable handling failed".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_platform_compatibility(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        if !platform_compatible {
            result.failures.push("Platform compatibility test failed".to_string());
            result.failed += 1;
            return Ok(());
        }
    ...
    async fn test_permission_systems(&self, result: &mut TestCategoryResult) -> Result<()> {
        ...
        if !permission_handling_correct {
            result.failures.push("Permission system integration failed".to_string());
            result.failed += 1;
            return Ok(());
        }
